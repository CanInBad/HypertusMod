...
func getStartActions(_sexEngine: SexEngine, _domInfo: SexDomInfo, _subInfo: SexSubInfo):
    ...
    if dom.isPlayer():
        for gun in dom.getInventory().getAllOf("hypertusRayGun"):
            for amount in range(dom.getBodypart(BodypartSlot.Breasts).getBaseSize()+5,max(dom.getBodypart(BodypartSlot.Breasts).getBaseSize()-5,0),-1):
                var toName = ""
                var toDesc = ""
                if amount-oldSize>0:
...
func processTurn():
    ...
    if state == "":
        ...
        if(tick>1):
            ...
            var toText = "{dom.You} shot {sub.yourHis} "+str(dom.getBodypart(BodypartSlot.Breast).getLewdName())+"."\
            +"\nNew size: "+breastSizeMod.breastSizeToCupString(dom.getBodypart(BodypartSlot.Breast).getsize())+number
